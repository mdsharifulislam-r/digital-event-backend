...
import { saveActivity } from "../../../handlers/activityHelper";
    ...
    await kafkaConsumer({groupId:"utils",topic:"utils",cb:async(data:{type:string,data:any})=>{
        try {
            ...
            switch (data.type) {
                ...
                case 'activity':
                    await saveActivity(data.data);
                    break;

                case 'push-notification':
                    await NotificationService.sendPushNotification(data.data.payload as any, data.data.user as any);
                    break;
